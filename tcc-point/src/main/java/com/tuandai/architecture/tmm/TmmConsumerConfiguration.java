...
public class TmmConsumerConfiguration {
	...
	public DeclareDeadQueue createAmqpConfig() {
		return new DeclareDeadQueue("tccpoint", "myVhost2", "tccExchange");
	}
	...
	@Bean
	public Binding bingdingqueue(@Qualifier("tccpoint") Queue queue,
			@Qualifier("mychange") TopicExchange topicExchange) {
		return BindingBuilder.bind(queue).to(topicExchange).with("#.tcc-point/point.#");
	}
}
